...
internal class CustomStandardShaderGUI : ShaderGUI
{
	...
	public override void OnGUI (MaterialEditor materialEditor, MaterialProperty[] props)
	{
		...
		CustomOnGUI(materialEditor, props);
	}

	public void CustomOnGUI(MaterialEditor materialEditor, MaterialProperty[] props)
	{
		GUILayout.Label("MainTex UV Mirror Setting (Custom)", EditorStyles.boldLabel);

		Material targetMat = materialEditor.target as Material;

		// see if mirror_u is set, and show a checkbox
		bool mirror_u = Array.IndexOf(targetMat.shaderKeywords, "MIRROR_U") != -1;
		EditorGUI.BeginChangeCheck();
		mirror_u = EditorGUILayout.Toggle("Mirror U", mirror_u);
		if (EditorGUI.EndChangeCheck())
		{
			// enable or disable the keyword based on checkbox
			if (mirror_u)
				targetMat.EnableKeyword("MIRROR_U");
			else
				targetMat.DisableKeyword("MIRROR_U");
		}

		// see if mirror_v is set, and show a checkbox
		bool mirror_v = Array.IndexOf(targetMat.shaderKeywords, "MIRROR_V") != -1;
		EditorGUI.BeginChangeCheck();
		mirror_v = EditorGUILayout.Toggle("Mirror V", mirror_v);
		if (EditorGUI.EndChangeCheck())
		{
			// enable or disable the keyword based on checkbox
			if (mirror_v)
				targetMat.EnableKeyword("MIRROR_V");
			else
				targetMat.DisableKeyword("MIRROR_V");
		}
	}
